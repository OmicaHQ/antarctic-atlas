...
def resource_path(*parts):
    candidates = [
        Path(getattr(sys, "_MEIPASS", "") or Path(__file__).resolve().parent),
    ...
    for keyword in sorted(set(keywords), key=len, reverse=True):
        if len(keyword) > 1:
            escaped = re.sub(
                ...
                lambda match: f"<span style='background-color:#7a5a13; color:#fff7d6; font-weight:700'>{match.group(0)}</span>",
                escaped,
                flags=re.IGNORECASE,
            )
    return escaped
...
def load_pdf_pages():
    if not PDF_PATH.exists():
        raise FileNotFoundError(f"Source PDF not found: {PDF_FILENAME}")
...
def search_pages(pages, query, max_results=12):
    keywords = [word.lower() for word in re.findall(r"[\w-]+", query) if len(word) > 1]
    if not keywords:
        return []
    results = []
    for page in pages:
...
def scored_search_pages(pages, query, max_results=5):
    keywords = [word.lower() for word in re.findall(r"[\w-]+", query) if len(word) > 1]
    if not keywords:
        return []
    results = []
...
def extract_paper_keywords(text):
    raw = str(text or "")
...
def is_low_value_reference_page(text):
...
def is_overview_question(text):
    ...
    overview_terms = ["overview", "summary", "summarize", "paper about", "main idea", "综述", "论文", "主要", "概括", "总结", "讲了什么"]
    return any(term in lowered for term in overview_terms)


def scored_search_pages_by_keywords(pages, keywords, max_results=5):
...
def clean_answer_markdown(text):
    cleaned = str(text or "")
...
RESEARCH_AREAS = {
    "Ocean": {
        "color": "#4EA3F1",
        "question": "How does Southern Ocean heat reach ice-shelf cavities?",
        "why": "Controls basal melting and ice-shelf thinning.",
            "topics": {
            "CDW Intrusion": "Warm Circumpolar Deep Water can access vulnerable continental shelf sectors.",
            "Cross-shelf Heat Transport": "Winds, eddies, tides, and bathymetry move heat toward ice shelves.",
            "Ice-shelf Basal Melt": "Basal melt thins shelves and weakens buttressing.",
            "Freshwater Feedback": "Meltwater freshening can alter stratification and future ocean heat access.",
        },
    },
    "Ice Dynamics": {
        "color": "#FF8A65",
        "question": "How does ice flow accelerate after ice shelves weaken?",
        "why": "Connects local forcing to large-scale ice discharge.",
        "topics": {
            "Buttressing": "Ice shelves resist inland ice flow; loss of support accelerates discharge.",
            "Grounding Line Retreat": "The grounded-to-floating transition strongly controls ice loss.",
            "MISI": "Marine Ice Sheet Instability may make retreat self-sustaining on retrograde beds.",
            "MICI": "Marine Ice Cliff Instability is a debated high-end sea-level mechanism.",
            "Basal Sliding": "Basal water and sediment conditions regulate fast ice streams.",
        },
    },
    "Solid Earth": {
        "color": "#9CCC65",
        "question": "How do bedrock, heat flow, and rebound affect ice stability?",
        "why": "Sets boundary conditions and feedbacks for ice-sheet retreat.",
        "topics": {
            "GIA": "Glacial Isostatic Adjustment can alter relative sea level near grounding lines.",
            "Bed Topography": "Bed geometry controls MISI-like retreat potential.",
            "Geothermal Heat Flux": "Sparse heat-flow observations affect basal meltwater and sliding estimates.",
            "Subglacial Hydrology": "Subglacial water can lubricate the bed and modify fast ice flow.",
        },
    },
    "Observations": {
        "color": "#9575CD",
        "question": "How do we measure change in such a remote environment?",
        "why": "Provides constraints for mechanisms and models.",
        "topics": {
            "Satellite Altimetry": "Tracks surface elevation change across Antarctica.",
            "InSAR Velocity": "Maps glacier speed and acceleration.",
            "GRACE / GRACE-FO": "Measures gravity change related to total ice mass.",
            "Radar & Field Data": "Reveals bed topography, internal layers, and basal conditions.",
        },
    },
    "Paleoclimate": {
        "color": "#F6C85F",
        "question": "What did the AIS do in past warm periods?",
        "why": "Extends evidence beyond the short satellite record.",
        "topics": {
            "Pliocene": "Warm-period evidence tests long-term ice-sheet sensitivity.",
            "Last Interglacial": "Past high sea levels constrain Antarctic contribution.",
            "Ice Cores": "Core records preserve atmosphere, temperature, and accumulation history.",
            "Marine Sediments": "Shelf and ocean archives reconstruct former ice-margin retreat.",
        },
    },
    "Future Projections": {
        "color": "#2F5597",
        "question": "How much will Antarctica contribute to future sea-level rise?",
        "why": "Connects science to coastal risk.",
        "topics": {
            "Sea-level Contribution": "The central societal impact of Antarctic ice-sheet change.",
            "Coupled Models": "Feedbacks require ice, ocean, atmosphere, and solid Earth coupling.",
            "Uncertainty Quantification": "Identifies which processes dominate projection spread.",
            "AI for Earth Observation": "AI can organize observations and detect patterns in ice-sheet change.",
        },
    },
}

UNIVERSE_TOPIC_KEYWORDS = {
    "Ocean": ["ocean", "southern ocean", "cdw", "basal melt", "heat transport", "warm water", "ocean forcing"],
    "CDW Intrusion": ["cdw", "circumpolar deep water", "intrusion", "warm deep water", "amundsen", "bellingshausen", "totten"],
    "Cross-shelf Heat Transport": ["cross shelf", "cross-shelf", "heat transport", "eddy", "eddies", "tide", "winds", "bathymetry", "shelf break"],
    "Ice-shelf Basal Melt": ["basal melt", "ice shelf melt", "melting from below", "sub ice shelf", "sub-ice-shelf", "cavity", "ocean heat"],
    "Freshwater Feedback": ["freshwater", "meltwater", "freshening", "stratification", "feedback"],
    "Ice Dynamics": ["ice dynamics", "ice flow", "grounding line", "buttressing", "misi", "mici", "basal sliding"],
    "Buttressing": ["buttressing", "back stress", "backstress", "ice shelf support", "pinning point"],
    "Grounding Line Retreat": ["grounding line", "grounding zone", "retreat", "migration", "grounded-to-floating"],
    "MISI": ["misi", "marine ice sheet instability", "retrograde bed", "self sustaining", "self-sustaining"],
    "MICI": ["mici", "marine ice cliff instability", "ice cliff", "hydrofracture", "cliff failure"],
    "Basal Sliding": ["basal sliding", "basal slip", "sliding", "friction", "till", "water pressure", "basal water", "sediment"],
    ...
    "Satellite Altimetry": ["altimetry", "icesat", "cryosat", "elevation", "surface height", "surface lowering"],
    "InSAR Velocity": ["insar", "sar", "velocity", "ice velocity", "interferometry", "speed"],
    "GRACE / GRACE-FO": ["grace", "grace-fo", "gravity", "mass balance", "gravimetry", "correction", "gia correction"],
    ...
    "Future Projections": ["future", "projection", "sea level", "uncertainty", "model", "2100", "2300"],
...
class UniverseMapWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.focused_topic = "Basal Sliding"
        self.nodes = self._build_nodes()
        self.stars = [((i * 37) % 100 / 100, (i * 61) % 100 / 100, 1 + (i % 3)) for i in range(88)]
        self._display_positions = {}
        self._animation_from = {}
        self._animation_to = {}
        self._animation_started = 0.0
        self._animation_duration = 0.85
        self._pulse_topic = None
    ...
    def _build_nodes(self):
        nodes = [{"name": "Antarctic Ice Sheet", "parent": None, "level": 0, "angle": 0, "radius": 0, "color": "#DDEEFF"}]
        area_angles = {
            "Ocean": 160,
            "Ice Dynamics": 25,
            "Solid Earth": 228,
            "Observations": 92,
            "Paleoclimate": 286,
            "Future Projections": 318,
        }
        for area_name, area in RESEARCH_AREAS.items():
            angle = area_angles.get(area_name, 0)
            nodes.append(
                {
                    "name": area_name,
                    "parent": "Antarctic Ice Sheet",
                    "level": 1,
                    "angle": angle,
                    "radius": 0.27,
                    "color": area["color"],
            ...
            topics = list(area["topics"])
            for index, topic_name in enumerate(topics):
                offset = (index - (len(topics) - 1) / 2) * 18
                nodes.append(
                    {
                        "name": topic_name,
                        "parent": area_name,
    ...
    def set_focus(self, topic, pulse=False):
        if topic:
            self._ensure_display_positions()
            self._animation_from = dict(self._display_positions)
            self.focused_topic = topic
            self._animation_to = self._layout_positions_for(topic)
            self._animation_started = time.monotonic()
            if pulse:
                self._pulse_topic = topic
                self._pulse_started = self._animation_started
            self._animation_timer.start()
            self.topicSelected.emit(topic)
            self.update()

    def _ensure_display_positions(self):
        if not self._display_positions or len(self._display_positions) != len(self.nodes):
            self._display_positions = self._layout_positions_for(self.focused_topic)

    def _advance_animation(self):
        if not self.isVisible():
            self._animation_timer.stop()
            return
        if not self._animation_to:
            self._animation_timer.stop()
            return
        ...
        for node in self.nodes:
            name = node["name"]
            start = self._animation_from.get(name, self._animation_to.get(name, QPointF()))
            end = self._animation_to.get(name, start)
            next_positions[name] = QPointF(
                start.x() + (end.x() - start.x()) * eased,
                start.y() + (end.y() - start.y()) * eased,
            )
        self._display_positions = next_positions
        if t >= 1:
            self._display_positions = dict(self._animation_to)
            self._animation_timer.stop()
        self.update()

    def _layout_positions_for(self, topic):
        return {node["name"]: self._node_position_for_topic(node, topic) for node in self.nodes}

    def _node_position_for_topic(self, node, topic):
        rect = self.rect()
        if topic != "Antarctic Ice Sheet":
            focused_layout = self._focused_node_positions(topic)
            if node["name"] in focused_layout:
                x, y = focused_layout[node["name"]]
                return QPointF(rect.width() * x, rect.height() * y)
        cx = rect.width() * 0.42
        cy = rect.height() * 0.51
        spread = min(rect.width(), rect.height()) * 0.66
        angle = (node["angle"] - 90) * pi / 180
        return QPointF(cx + cos(angle) * spread * node["radius"], cy + sin(angle) * spread * node["radius"])

    def _focused_node_positions(self, topic=None):
        topic = topic or self.focused_topic
        node_by_name = {node["name"]: node for node in self.nodes}
        focused = node_by_name.get(topic)
        if not focused or topic == "Antarctic Ice Sheet":
            return {}
        if focused["level"] == 1:
            anchor = focused["name"]
            children = [node["name"] for node in self.nodes if node.get("parent") == anchor]
            current = anchor
        else:
            anchor = focused.get("parent", topic)
            children = [node["name"] for node in self.nodes if node.get("parent") == anchor]
            current = focused["name"]
        positions = {
            "Antarctic Ice Sheet": (0.57, 0.74),
            anchor: (0.18, 0.44),
        }
        if children:
            siblings = [child for child in children if child != current]
            if current != anchor:
                positions[current] = (0.40, 0.47)
            sibling_slots = [(0.15, 0.58), (0.28, 0.47), (0.50, 0.56), (0.50, 0.36), (0.30, 0.62)]
            for index, child in enumerate(siblings):
                positions[child] = sibling_slots[index % len(sibling_slots)]
        return positions

    def _node_size(self, node):
        return {0: 50, 1: 32}.get(node["level"], 19)
    ...
    def paintEvent(self, event):
        ...
        background.setColorAt(0, QColor("#061427"))
        background.setColorAt(0.52, QColor("#07111f"))
        background.setColorAt(1, QColor("#020617"))
        painter.fillRect(rect, background)

        for center, color in [
            (QPointF(rect.width() * 0.28, rect.height() * 0.26), QColor(78, 163, 241, 70)),
            (QPointF(rect.width() * 0.78, rect.height() * 0.68), QColor(149, 117, 205, 56)),
        ]:
            glow = QRadialGradient(center, min(rect.width(), rect.height()) * 0.36)
        ...
        for sx, sy, size in self.stars:
            painter.setBrush(QColor(240, 250, 255, 130))
            painter.drawEllipse(QPointF(rect.width() * sx, rect.height() * sy), size * 0.55, size * 0.55)

        positions = self._display_positions
        related = self._related_topics(self.focused_topic)
        for node in self.nodes:
            if not node["parent"]:
                continue
            active = node["name"] in related and node["parent"] in related
            pen = QPen(QColor(126, 220, 255, 150 if active else 34), 2.4 if active else 1.0)
            if self.focused_topic != "Antarctic Ice Sheet" and active:
                pen.setStyle(Qt.DashLine)
            painter.setPen(pen)
            painter.drawLine(positions[node["parent"]], positions[node["name"]])

        def node_draw_priority(node):
            if node["name"] == self.focused_topic:
                return 2
            if node["name"] in related:
                return 1
            return 0

        for node in sorted(self.nodes, key=node_draw_priority):
            pos = positions[node["name"]]
            size = self._node_size(node)
            active = node["name"] == self.focused_topic
            dim = self.focused_topic != "Antarctic Ice Sheet" and node["name"] not in related
            color = QColor(node["color"])
            color.setAlpha(235 if not dim else 78)

            outer = QRadialGradient(pos - QPointF(size * 0.28, size * 0.34), size * 1.2)
            outer.setColorAt(0, QColor(255, 255, 255, 150 if not dim else 42))
            outer.setColorAt(0.32, color)
            outer.setColorAt(1, QColor(7, 14, 29, 120 if not dim else 50))
            painter.setBrush(outer)
            painter.setPen(QPen(QColor(255, 255, 255, 220 if active else 125), 3 if active else 1.4))
            painter.drawEllipse(pos, size, size)
            if node["name"] == self._pulse_topic:
                pulse_elapsed = time.monotonic() - self._pulse_started
                if pulse_elapsed < 1.25:
                    pulse_t = pulse_elapsed / 1.25
                    ring = size + 15 + pulse_t * 34
                    alpha = int(190 * (1 - pulse_t))
                    painter.setBrush(Qt.NoBrush)
                    painter.setPen(QPen(QColor(126, 220, 255, alpha), 4))
                    painter.drawEllipse(pos, ring, ring)
                    if not self._animation_timer.isActive():
                        QTimer.singleShot(FRAME_INTERVAL_MS, self.update)
                else:
                    self._pulse_topic = None

            show_label = node["level"] <= 1
            if node["level"] == 2:
                show_label = (
                    node["name"] == self.focused_topic
                    or (
                        self.focused_topic != "Antarctic Ice Sheet"
                        and (
                            node["parent"] == self.focused_topic
                            or node["name"] in related
                        )
                    )
                )

            if show_label:
                painter.setPen(QColor(246, 251, 255, 230 if not dim else 52))
                font = QFont("Segoe UI", 8 if node["level"] == 2 else 11, QFont.Bold)
                painter.setFont(font)
                label_height = 28 if node["level"] == 2 else 36
                label_rect = QRectF(pos.x() - size * 1.95, pos.y() + size + 3, size * 3.9, label_height)
                painter.drawText(label_rect, Qt.AlignHCenter | Qt.AlignTop | Qt.TextWordWrap, node["name"])

        painter.setPen(Qt.NoPen)
        painter.setBrush(QColor(5, 13, 27, 170))
        painter.drawRoundedRect(QRectF(22, 22, 330, 96), 18, 18)
        painter.setPen(QColor("#f8fbff"))
        painter.setFont(QFont("Segoe UI", 17, QFont.Bold))
        painter.drawText(QRectF(42, 36, 300, 30), "Antarctic Research Universe")
        painter.setPen(QColor(221, 240, 252, 185))
        painter.setFont(QFont("Segoe UI", 10))
        painter.drawText(QRectF(42, 68, 285, 42), Qt.TextWordWrap, "Click a sphere or ask below; the matching research node stays in focus.")

        self._draw_detail_card(painter, rect)
        hint = QRectF(24, rect.height() - 40, min(420, rect.width() - 48), 24)
        painter.setBrush(QColor(2, 6, 23, 76))
        painter.setPen(QPen(QColor(210, 238, 255, 24), 1))
        painter.drawRoundedRect(hint, 14, 14)
        painter.setPen(QColor(221, 240, 252, 142))
        painter.setFont(QFont("Segoe UI", 8))
        painter.drawText(hint.adjusted(12, 0, -12, 0), Qt.AlignVCenter, "Click a sphere · Ask below · matched module auto-focuses here")

    def _related_topics(self, topic):
        related = {topic}
        node_by_name = {node["name"]: node for node in self.nodes}
        node = node_by_name.get(topic)
        if not node:
            return related
        if node["parent"]:
            related.add(node["parent"])
        for candidate in self.nodes:
            if candidate["parent"] == topic or candidate["parent"] == node.get("parent"):
                related.add(candidate["name"])
        if node["level"] == 2:
            related.add("Antarctic Ice Sheet")
        return related

    def _topic_detail(self, name):
        topic_meta = {
            "CDW Intrusion": ("Active frontier", "Amundsen Sea, Bellingshausen Sea, Totten Glacier"),
            "Cross-shelf Heat Transport": ("High uncertainty", "Antarctic continental shelf"),
            "Ice-shelf Basal Melt": ("Observed but hard to model", "Pine Island, Thwaites, Totten"),
            "Freshwater Feedback": ("Emerging feedback", "Southern Ocean"),
            "Buttressing": ("Core mechanism", "Antarctic Peninsula, Amundsen Sea"),
            "Grounding Line Retreat": ("Central research target", "Thwaites, Pine Island, Totten"),
            "MISI": ("High-impact uncertainty", "WAIS, Wilkes Subglacial Basin"),
            "MICI": ("Debated mechanism", "Potential marine-based Antarctic margins"),
            "Basal Sliding": ("Difficult to observe", "Fast-flowing outlet glaciers"),
            "GIA": ("Important feedback", "West Antarctica"),
            "Bed Topography": ("Critical boundary data", "Thwaites, Wilkes, Aurora, Sabrina"),
            "Geothermal Heat Flux": ("Sparse observations", "West Antarctica, South Pole region"),
            "Subglacial Hydrology": ("Hard-to-access frontier", "Subglacial lakes and drainage systems"),
            "Satellite Altimetry": ("Mature remote sensing tool", "Continent-wide"),
            "InSAR Velocity": ("Highly relevant remote sensing layer", "Fast outlet glaciers"),
            "GRACE / GRACE-FO": ("Powerful but needs GIA correction", "Continent-wide"),
            "Radar & Field Data": ("Essential but incomplete", "Ice streams, grounding zones, subglacial basins"),
            "Pliocene": ("Important but uncertain", "WAIS and marine-based EAIS"),
            "Last Interglacial": ("Useful constraint", "Antarctic margins"),
            "Ice Cores": ("Foundational evidence", "Interior Antarctica"),
            "Marine Sediments": ("Key paleo archive", "Continental shelf and deep ocean"),
            "Sea-level Contribution": ("Uncertain but crucial", "Global coastlines"),
            "Coupled Models": ("Major modeling frontier", "Antarctica and global climate system"),
            "Uncertainty Quantification": ("High priority", "Model ensembles"),
            "AI for Earth Observation": ("Emerging opportunity", "Remote sensing and research synthesis"),
        }
        if name == "Antarctic Ice Sheet":
            return (
                "Core system",
                "Antarctic Ice Sheet",
                "The central system linking atmosphere, ocean, ice dynamics, solid Earth, observations, paleoclimate evidence, and sea-level risk.",
                "Antarctica is the major long-term uncertainty in sea-level rise.",
                "Atmosphere, ocean, solid Earth, observations, projections",
            )
        if name in RESEARCH_AREAS:
            area = RESEARCH_AREAS[name]
            return ("Research area", name, area["question"], area["why"], " - ".join(area["topics"].keys()))
        parent = next((area for area, meta in RESEARCH_AREAS.items() if name in meta["topics"]), "Research area")
        detail = RESEARCH_AREAS.get(parent, {}).get("topics", {}).get(name, "")
        status, regions = topic_meta.get(name, ("Research frontier", parent))
        return (parent, name, detail, status, regions)

    def _draw_detail_card(self, painter, rect):
        card = QRectF(rect.width() * 0.58, 54, rect.width() * 0.36, min(430, rect.height() - 108))
        painter.setPen(QPen(QColor(210, 238, 255, 72), 1.2))
        card_gradient = QLinearGradient(card.topLeft(), card.bottomRight())
        card_gradient.setColorAt(0, QColor(17, 35, 62, 226))
        card_gradient.setColorAt(1, QColor(5, 13, 27, 188))
        painter.setBrush(card_gradient)
        painter.drawRoundedRect(card, 22, 22)

        badge, title, question, why, linked = self._topic_detail(self.focused_topic)
        badge_rect = QRectF(card.left() + 22, card.top() + 22, min(155, card.width() - 44), 29)
        painter.setPen(QPen(QColor(126, 220, 255, 60), 1))
        painter.setBrush(QColor(78, 163, 241, 40))
        ...
        painter.drawText(QRectF(card.left() + 22, card.top() + 70, card.width() - 44, 38), Qt.TextWordWrap, title)
        painter.setPen(QColor("#9ed8f5"))
        painter.setFont(QFont("Segoe UI", 8, QFont.Bold))
        painter.drawText(QRectF(card.left() + 22, card.top() + 124, card.width() - 44, 20), "KEY QUESTION")
        painter.setPen(QColor(245, 250, 255, 220))
        painter.setFont(QFont("Segoe UI", 10))
        painter.drawText(QRectF(card.left() + 22, card.top() + 146, card.width() - 44, 82), Qt.TextWordWrap, question)
        painter.setPen(QColor("#9ed8f5"))
        painter.setFont(QFont("Segoe UI", 8, QFont.Bold))
        second_label = "WHY IT MATTERS" if self.focused_topic == "Antarctic Ice Sheet" or self.focused_topic in RESEARCH_AREAS else "RESEARCH STATUS"
        painter.drawText(QRectF(card.left() + 22, card.top() + 242, card.width() - 44, 20), second_label)
        painter.setPen(QColor(245, 250, 255, 220))
        painter.setFont(QFont("Segoe UI", 10))
        painter.drawText(QRectF(card.left() + 22, card.top() + 264, card.width() - 44, 58), Qt.TextWordWrap, why)
        painter.setPen(QColor("#9ed8f5"))
        painter.setFont(QFont("Segoe UI", 8, QFont.Bold))
        painter.drawText(QRectF(card.left() + 22, card.top() + 338, card.width() - 44, 20), "KEY REGIONS / LINKED TOPICS")
        painter.setPen(QColor(245, 250, 255, 210))
        painter.setFont(QFont("Segoe UI", 9))
        painter.drawText(QRectF(card.left() + 22, card.top() + 360, card.width() - 44, 48), Qt.TextWordWrap, linked)
    ...
    def mousePressEvent(self, event):
        ...
        for node in self.nodes:
            pos = self._display_positions.get(node["name"], self._node_position_for_topic(node, self.focused_topic))
            distance = ((click.x() - pos.x()) ** 2 + (click.y() - pos.y()) ** 2) ** 0.5
            if distance < nearest_distance:
                nearest_name = node["name"]
                nearest_distance = distance
        if nearest_name and nearest_distance <= self._node_size(next(node for node in self.nodes if node["name"] == nearest_name)) + 18:
            self.set_focus(nearest_name)
...
class NativeAtlasWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.shell.currentChanged.connect(self._on_shell_changed)
    ...
    def _on_shell_changed(self, index):
        return
